...
const sliderEscalaVista = document.getElementById("escalaVista");
const valorEscalaVista = document.getElementById("valorEscalaVista");

sliderEscalaVista.addEventListener("input", () => {
    escalaVista = sliderEscalaVista.value / 100;
    valorEscalaVista.textContent = sliderEscalaVista.value + "%";
    redibujarImagen();
});

document.getElementById("rotarIzquierda").addEventListener("click", () => {
    rotacion -= 90;
    redibujarImagen();
});

document.getElementById("rotarDerecha").addEventListener("click", () => {
    rotacion += 90;
    redibujarImagen();
});

function redibujarImagen() {
    if (!originalImage) return;

    const fuente = currentImage || originalImage;

    const tempCanvas = document.createElement("canvas");
    const tempCtx = tempCanvas.getContext("2d");

    tempCanvas.width = fuente.width;
    tempCanvas.height = fuente.height;

    //tempCtx.putImageData(originalImage, 0, 0);
    tempCtx.putImageData(currentImage || originalImage, 0, 0);
    ...
    if (rotacion % 180 !== 0) {
        canvas.width = originalImage.height * escalaVista;
        canvas.height = originalImage.width * escalaVista;
    } else {
        canvas.width = originalImage.width * escalaVista;
        canvas.height = originalImage.height * escalaVista;
    }
    ...
    // Escalar
    ctx.scale(escalaVista, escalaVista);
